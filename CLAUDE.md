...
1. **Ward boundaries (primary)** — Western Cape Government ArcGIS MapServer, queried as GeoJSON. If this fails, the whole map fails with an error box (`map.remove()`).
...
**Ward numbering:** the human ward number (1–23) maps to a `WARD_NO` string via `CONFIG.wardNoPrefix` (`"10204"`) + zero-padded number, e.g. ward 11 → `"10204011"`. Use `buildWardNo()` / `wardNumberFromWardNo()`; JSON data keys on the full `wardid`.
